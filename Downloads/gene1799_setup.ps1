...
# 2. VERIFICA FILE ESSENZIALI (usa codice da paste.txt se mancanti)
if (-not (Test-Path "server.js")) {
    Write-Host "`n⚠️  server.js mancante. Creando da template base..." -ForegroundColor Yellow
    @"
...
app.get('/api/agents', (req, res) => {
    res.json({
        agents: [
            { id: 1, name: 'Gene1799-Assistant', status: 'active', capabilities: ['chat', 'analysis'] }
        ],
        count: 1
    });
});
...
app.listen(PORT, () => {
    console.log(`🚀 Gene1799 Backend v6.0.0 ready on port ${PORT}`);
