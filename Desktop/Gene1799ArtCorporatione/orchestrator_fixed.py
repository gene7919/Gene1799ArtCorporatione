...
# Configure logging - use ASCII only to avoid encoding issues on Windows
logging.basicConfig(
    level=logging.INFO,
...
class GeneOrchestrator:
    ...
    def __init__(self, project_root: Path = None):
        ...
        logger.info("[OK] Gene1799 Master Orchestrator initialized")
    ...
    def _initialize_services(self):
        ...
        logger.info("[OK] Initialized %d services" % len(self.services))
    ...
    def _initialize_agents(self):
        ...
        logger.info("[OK] Initialized %d AI agents" % len(self.agents))

    async def start_service(self, service_name: str) -> bool:
        """Start a single service"""
        if service_name not in self.services:
            logger.error("[ERROR] Service '%s' not found" % service_name)
            return False
        ...
        for dep in service.depends_on:
            if dep not in self.services or self.services[dep].status != ServiceStatus.RUNNING:
                logger.warning("[WARN] Waiting for dependency: %s" % dep)
                await asyncio.sleep(2)

        logger.info("[START] Starting service: %s" % service.name)
        ...
        try:
            ...
            logger.info("[OK] Started: %s (port %s)" % (service.name, service.port))
            return True
        except Exception as e:
            logger.error("[ERROR] Failed to start %s: %s" % (service.name, str(e)))
    ...
    async def stop_service(self, service_name: str) -> bool:
        ...
        service = self.services[service_name]
        logger.info("[STOP] Stopping service: %s" % service.name)
        ...
        try:
            ...
            logger.info("[OK] Stopped: %s" % service.name)
            return True
        except Exception as e:
            logger.error("[ERROR] Failed to stop %s: %s" % (service.name, str(e)))
            service.status = ServiceStatus.ERROR
            return False
    ...
    async def start_all_services(self):
        ...
        logger.info("=" * 60)
        logger.info("[START] STARTING ALL SERVICES")
        logger.info("=" * 60)
        ...
        for service_name in startup_order:
            if service_name in self.services:
                ...
                if not success and service_name in ['backend', 'ai-agent']:
                    logger.error("[ERROR] Critical service failed: %s" % service_name)
                    return False
                await asyncio.sleep(2)

        self.is_running = True
        logger.info("=" * 60)
        logger.info("[OK] ALL SERVICES RUNNING")
        logger.info("=" * 60)
        return True
    ...
    async def stop_all_services(self):
        ...
        logger.info("=" * 60)
        logger.info("[STOP] STOPPING ALL SERVICES")
        logger.info("=" * 60)
    ...
    async def initialize_agents(self) -> bool:
        ...
        logger.info("=" * 60)
        logger.info("[INIT] INITIALIZING AI AGENTS")
        logger.info("=" * 60)

        for agent_name, agent in self.agents.items():
            logger.info("[START] Initializing agent: %s" % agent.name)
            try:
                # Simulate agent initialization
                await asyncio.sleep(0.5)
                agent.status = ServiceStatus.RUNNING
                agent.last_heartbeat = datetime.now()
                logger.info("[OK] Initialized: %s" % agent.name)
                logger.info("    Role: %s" % agent.role)
                logger.info("    Capabilities: %s" % ', '.join(agent.capabilities))
            except Exception as e:
                logger.error("[ERROR] Failed to initialize agent %s: %s" % (agent.name, str(e)))
    ...
    def print_status(self):
        ...
        for name, service in self.services.items():
            status_icon = {
                ...
                ServiceStatus.ERROR: "[ERR]",
                ServiceStatus.STARTING: "[START]",
                ServiceStatus.STOPPING: "[STOP]",
            ...
            if service.start_time:
                ...
                uptime = " (uptime: %ds)" % delta.seconds

            logger.info("%s %s | %s | port %s%s" % (status_icon, service.name, service.status.value, service.port, uptime))

        logger.info("\nAI Agents:")
        for name, agent in self.agents.items():
            status_icon = "[ACTIVE]" if agent.status == ServiceStatus.RUNNING else "[INACTIVE]"
            logger.info("%s %s | %s" % (status_icon, agent.name, agent.role))
        ...
        while True:
            try:
                ...
                if cmd == 'start':
                    ...
                    for name, agent in self.agents.items():
                        logger.info("  * %s (%s): %s" % (agent.name, agent.agent_type, agent.role))
                    ...
                    logger.warning("[WARN] Unknown command: %s" % cmd)
