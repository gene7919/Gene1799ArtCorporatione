...
$runPath = Join-Path $root "Run-MainApp.ps1"
...
foreach ($file in @($mainPath, $runPath, $resetPath, $testPath)) {
    if (Test-Path $file) {
        $backupName = "{0}_{1}.bak" -f ([IO.Path]::GetFileName($file)), (Get-Date -Format "yyyyMMdd_HHmmss")
        Copy-Item $file (Join-Path $backupDir $backupName) -Force
    }
}
...
function Get-GroqApiKey {
    ...
    if (Test-Path $secretPath) {
        try {
            ...
            Write-Log "Archivio chiave locale non leggibile. Verrà richiesta una nuova chiave." "WARN"
        }
    }
...
Write-Host ""
Write-Host "PATCH GROQ PRODUZIONE COMPLETATA" -ForegroundColor Green
...
Write-Host " - $testPath"
Write-Host ""
Write-Host "Backup in: $backupDir" -ForegroundColor Yellow
Write-Host "Log in:    $logsDir" -ForegroundColor Yellow
Write-Host "Output in: $outputDir" -ForegroundColor Yellow
Write-Host ""
Write-Host "Comandi utili:" -ForegroundColor Cyan
Write-Host "cd C:\SuiteV17"
Write-Host ".\Run-MainApp.ps1"
Write-Host '.\Run-MainApp.ps1 -Prompt "Scrivi un post social energico su Gene1799artcorporatione."'
Write-Host ".\Test-Groq.ps1"
Write-Host ".\Reset-GroqKey.ps1"