...
Write-Host " SUITE V17 // CONTROL ROOM LITE STABILE " -ForegroundColor Cyan
Write-Host "=========================================" -ForegroundColor Cyan
...
$LaunchPs1   = Join-Path $AppDir "Launch-ControlRoom-Lite.ps1"
$LaunchBat   = Join-Path $AppDir "Launch-ControlRoom-Lite.bat"
$DesktopBat  = Join-Path ([Environment]::GetFolderPath("Desktop")) "SuiteV17 Control Room Lite.bat"
...
$packageJson = @'
{
  "name": "suitev17-control-room-lite",
  "version": "1.0.0",
  "description": "Suite V17 Control Room Lite stabile",
  "main": "main.js",
  "scripts": {
    "start": "electron ."
  },
...
function createWindow() {
  ...
  const win = new BrowserWindow({
    ...
    title: "Suite V17 // Control Room Lite",
...
function probeUrl(url, timeout = 2500) {
  ...
  return {
    ...
    modules: {
      token: {
        ...
        monitorExists: exists(PATHS.tokenMonitor),
        serverExists: exists(PATHS.tokenServer),
        data: tokenData,
        config: {
...
async function startTokenMonitor() {
  if (!exists(PATHS.tokenMonitor)) return { ok: false, error: "token_monitor.js non trovato" };
  return startNodeScript(".\\token_monitor.js", PATHS.tokenModule);
}

async function startTokenDashboardServer() {
  if (!exists(PATHS.tokenServer)) return { ok: false, error: "server.js non trovato" };
  return startNodeScript(".\\server.js", PATHS.tokenModule);
}

async function startBrowserModule() {
  if (!exists(PATHS.browserServer)) return { ok: false, error: "browser_server.js non trovato" };
...
async function testTelegram() {
  ...
  const payload = JSON.stringify({
    chat_id: chatId,
    text: "🔥 Suite V17 Control Room Lite: test Telegram OK"
  });
...
async function testWebhook() {
  ...
  const payload = JSON.stringify({
    platform: "telegram",
    title: "Suite V17 Control Room Lite Test",
    source: "Electron Lite",
    time: new Date().toISOString(),
    text: "✅ Webhook test da Suite V17 Control Room Lite"
  });
...
<head>
  ...
  <title>Suite V17 // Control Room Lite</title>
  <style>
    :root{
      --bg:#050806;
      --panel:rgba(8,14,10,.95);
      --line:#8dff2a;
      --text:#dbff9d;
      --muted:#7faa56;
      --ok:#79ff93;
    ...
    *{box-sizing:border-box}
    ...
    .grid{display:grid;grid-template-columns:320px 1fr 420px;gap:16px}
    .panel{padding:16px}
    ...
    .console,.logs{
      min-height:260px;max-height:480px;overflow:auto;white-space:pre-wrap;
...
<div class="wrap">
  <div class="topbar">
    <div>
      <div class="title">SUITE V17 // CONTROL ROOM LITE</div>
      <div class="sub">Versione stabile, senza PM2 e senza packaging</div>
    </div>
    ...
    <div class="stack">
      <div class="panel">
        ...
        <div class="btn-grid">
          ...
          <button id="openTokenDashBtn">Apri localhost:3018</button>
      ...
      <div class="panel">
        ...
        <div class="stack">
          ...
          <div class="service">
            ...
            <div class="mini" id="telegramMeta">Bot e chat ID</div>
          </div>
          <div class="service">
            <div class="service-row">
              <div><span class="dot" id="socialDot"></span>Social / Webhook</div>
              <div class="small" id="socialStatus">...</div>
            </div>
            <div class="mini" id="socialMeta">Make / n8n / webhook</div>
          </div>
        </div>
      </div>
    </div>
...
document.getElementById("openTokenDashBtn").addEventListener("click", () => window.suitev17.openExternal("http://127.0.0.1:3018/"));
document.getElementById("openBrowserHealthBtn").addEventListener("click", () => window.suitev17.openExternal("http://127.0.0.1:8093/health"));
...
$launchBatContent = @'
@echo off
powershell -NoProfile -ExecutionPolicy Bypass -File "C:\SuiteV17\ElectronApp\Launch-ControlRoom-Lite.ps1"
'@
...
Write-Host "File Control Room Lite creati." -ForegroundColor Green
...
Write-Host ""
Write-Host "=========================================" -ForegroundColor Cyan
Write-Host " PATCH CONTROL ROOM LITE COMPLETATA " -ForegroundColor Green
Write-Host "=========================================" -ForegroundColor Cyan
Write-Host "Launcher locale: C:\SuiteV17\ElectronApp\Launch-ControlRoom-Lite.bat"
Write-Host "Launcher Desktop: $DesktopBat"
Write-Host ""
Write-Host "Avvio da PowerShell:"
Write-Host '& "$HOME\Desktop\SuiteV17 Control Room Lite.bat"'